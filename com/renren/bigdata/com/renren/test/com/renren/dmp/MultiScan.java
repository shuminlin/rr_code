...
public class MultiScan {
    public static void main(String[] args)throws Exception{
        ...
        ScanThread[] c = new ScanThread[THREAD_AMOUNT];
        ...
        HTablePool pool = new HTablePool(conf,THREAD_AMOUNT);
        HTableInterface[] tables = new HTableInterface[THREAD_AMOUNT];
        for(int i=0;i<THREAD_AMOUNT;i++){
            tables[i] = pool.getTable("test_id_hub");
...
//            tables[i].setWriteBufferSize(4*1024*1024);
        ...
        for(int i=0;i<THREAD_AMOUNT;i++){
            c[i] = new ScanThread(countDownLatch,queue,pool);
